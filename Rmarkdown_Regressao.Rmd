...
# Autocorrelação entre os erros - Teste de Durbin-Watson

```{r}
car::durbinWatsonTest(model_final, max.lag = 1)
```
Como o p-valor é menor do que 0.05, rejeitamos H0 e supomos que há correlação entre os erros, mas não muito forte pois a estatística de teste D está próxima de 2.

Relembrando:
• 𝐷 ≈ 2: Não há autocorrelação.
• 𝐷 ≈ 0: Autocorrelação positiva.
• 𝐷 ≈ 4: Autocorrelação negativa.
...
Autocorrelação dos resíduos: AINDA PRESENTE